...
def _read_number_slider_values(ghx_path: Path) -> dict[str, str]:
    root_element = element_tree.parse(ghx_path).getroot()
    slider_values: dict[str, str] = {}
    for object_element in root_element.iter("chunk"):
        if object_element.get("name") != "Object":
            continue
        component_name = object_element.find('./items/item[@name="Name"]')
        if component_name is None or component_name.text != "Number Slider":
            continue
        container_element = object_element.find('./chunks/chunk[@name="Container"]')
        if container_element is None:
            continue
        instance_guid = container_element.find('./items/item[@name="InstanceGuid"]')
        slider_value = container_element.find(
            './chunks/chunk[@name="Slider"]/items/item[@name="Value"]'
        )
        if instance_guid is None or slider_value is None:
            continue
        slider_values[instance_guid.text] = slider_value.text
...
def test_gradient_transform_preserves_existing_number_slider_values(tmp_path: Path) -> None:
    source_ghx_path = gradient_source_ghx_path()
    assert source_ghx_path is not None

    source_slider_values = _read_number_slider_values(source_ghx_path)
    gradient_ghx_path = tmp_path / "definition_gradient.ghx"
    transform_penalty_graph_for_gradient(source_ghx_path, gradient_ghx_path)
    gradient_slider_values = _read_number_slider_values(gradient_ghx_path)

    for instance_guid, source_slider_value in source_slider_values.items():
        assert gradient_slider_values[instance_guid] == source_slider_value
    assert len(gradient_slider_values) == len(source_slider_values) + 1
